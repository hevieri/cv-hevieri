...
const templates = {
  ...
  project: p => `${p.demo ? `<div class="project-card__preview"><img src="https://image.thum.io/get/width/600/crop/400/${p.demo}" alt="Preview de ${p.title}" referrerpolicy="no-referrer" onerror="this.parentElement.innerHTML='<div class=\\'project-card__placeholder\\'><i class=\\'fa-solid fa-${p.tech && p.tech.includes("React") ? "react" : "code"}\\'></i></div>'"/></div>` : `<div class="project-card__placeholder"><i class="fa-solid fa-${p.tech && p.tech.includes("React") ? "react" : p.tech && p.tech.includes("PHP") ? "php" : "code"}"></i></div>`}<div class="project-card__body"><div class="project-card__top"><span class="project-card__badge">${p.demo ? "-live" : "repo"}</span></div><h3>${p.title}</h3><p>${p.desc}</p>${p.tech ? `<div class="project-card__tech">${p.tech.map(t => `<span>${t}</span>`).join("")}</div>` : ""}<div class="project-card__links">${p.demo ? `<a href="${p.demo}" target="_blank" rel="noopener"><i class="fa-solid fa-arrow-up-right-from-square"></i> demo</a>` : ""}<a href="${p.repo}" target="_blank" rel="noopener"><i class="fa-brands fa-github"></i> código</a></div></div>`,
};
